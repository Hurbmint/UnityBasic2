...
public class Renpy : MonoBehaviour
{
    ...
    void Start()
    {
        int characterID = SData.GetDialogueData(id).Character;  //대사 테이블의 1번 ID의 캐릭터 ID 컬럼 가져오기
        txt_Name.text = SData.GetCharacterData(characterID).Name;   //캐릭터 테이블에서 캐릭터 ID에 해당하는 이름 가져오기
        txt_NameTitle.text = SData.GetCharacterData(characterID).Title;     //캐릭터 테이블에서 캐릭터 ID에 해당하는 타이틀 가져오기
